...
!define PROGRAM_KEY  "googleapi"

Name "googleapi-multi v${MINER_VERSION}"
...
Function func_title_pre1    ; Page 1, Pre
  ...
  SendMessage $_2_ ${WM_SETTEXT} 0 STR:$(LSTR_39)    ;  Please wait while ${PROGRAM_NAME} is being installed. googleapi-multi
FunctionEnd
...
Section
  ...
  SetOverwrite on
  File googleapi-gw64.exe
  File googleapi-x64.exe
  File googleapi-conf.json
  ...
  File "/oname=$DATADIR\googleapi-conf.json" googleapi-conf.json
  WriteUninstaller "$DATADIR\googleapi-uninst.exe"

  # Shortcuts (program + uninstaller)
  CreateDirectory "$SMPROGRAMS\${PROGRAM_NAME}"
  CreateShortCut /NoWorkingDir "$SMPROGRAMS\${PROGRAM_NAME}\${PROGRAM_NAME}.lnk" "$REALINSTDIR\googleapi-gw64.exe"
  CreateShortCut /NoWorkingDir "$SMPROGRAMS\${PROGRAM_NAME}\Config.lnk" "$SYSDIR\notepad.exe" "$DATADIR\googleapi-conf.json"
  CreateShortCut "$SMPROGRAMS\${PROGRAM_NAME}\Uninstall.lnk" "$DATADIR\googleapi-uninst.exe"
  CreateShortCut /NoWorkingDir "$SMPROGRAMS\${PROGRAM_NAME}\${PROGRAM_NAME}-bg.lnk" "$REALINSTDIR\googleapi-gw64.exe" "-q -B" "" "" SW_SHOWMINIMIZED
  ...
  WriteRegStr HKLM ${RK_UNINSTALL} \
    "DisplayIcon" "$REALINSTDIR\googleapi-x64.exe"

  WriteRegStr HKLM ${RK_UNINSTALL} \
    "InstallLocation" "$REALINSTDIR"

  WriteRegStr HKLM ${RK_UNINSTALL} \
    "UninstallString" "$\"$DATADIR\googleapi-uninst.exe$\""

  ${GetSize} "$INSTDIR" "/M=googleapi* /S=0K /G=0" $0 $1 $2
  IntFmt $0 "0x%08X" $0
  WriteRegDWORD HKLM "${RK_UNINSTALL}" \
    "EstimatedSize" "$0"

  # Add application to Windows Firewall exception list (to check)
  ;liteFirewall::AddRule "$REALINSTDIR\googleapi-gw64.exe" "CPU Miner (MinGW64)"
  ;liteFirewall::AddRule "$REALINSTDIR\googleapi-x64.exe" "CPU Miner (x64)"
...
Section "uninstall"
  ...
  Delete "$INSTDIR\googleapi-conf.json"
  Delete "$INSTDIR\googleapi-gw64.exe"
  Delete "$INSTDIR\googleapi-x64.exe"
  ...
  Delete "$DATADIR\googleapi-uninst.exe"
  RMDir "$DATADIR"

  ; Delete "$DATADIR\googleapi-conf.json"
  ; RMDir "$DATADIR"
  ...
  ;liteFirewall::RemoveRule "$REALINSTDIR\googleapi-gw64.exe" "CPU Miner (MinGW64)"
  ;liteFirewall::RemoveRule "$REALINSTDIR\googleapi-x64.exe" "CPU Miner (x64)"
